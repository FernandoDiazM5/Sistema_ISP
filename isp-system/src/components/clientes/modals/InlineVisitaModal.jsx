...
export default function InlineVisitaModal({ client, motivo, onClose, onSuccess, diagnostico: initialDiag }) {
    ...
    useEffect(() => {
        if (client) {
            ...
            setVisitaDescripcion(`Derivación directa: ${motivo}`);
            setVisitaAdjuntos([]);
            setDiagnostico(initialDiag || getEmptyDiag());
            setSuccess(false);
        }
    }, [client, motivo, initialDiag]);
            ...
            <div className={`bg-bg-card rounded-2xl p-6 w-[640px] border max-h-[90vh] overflow-y-auto transition-colors duration-500 ${success ? 'border-green-500/60' : 'border-border'}`} onClick={e => e.stopPropagation()}>
                {success ? (
                    <div className="flex flex-col items-center justify-center py-12 gap-3">
                        ...
                        <p className="text-sm text-text-secondary">La visita fue agendada exitosamente.</p>
                    </div>
                ) : (
                    <>
                        <div className="flex items-center justify-between mb-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div><h3 className="text-lg font-bold">Agendar Visita Técnica</h3><p className="text-xs text-text-muted">Derivación desde cliente</p></div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 gap-3 mb-5">
                            <div className="bg-bg-secondary rounded-lg p-3 border border-border/50">
                                ...
                                <p className="text-[11px] text-text-muted mt-0.5">Motivo: {motivo}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="date" value={visitaFecha} onChange={e => setVisitaFecha(e.target.value)} className="w-full min-h-[38px] px-3 bg-bg-secondary border border-border rounded-lg text-sm" />
                                </div>
                                <div>
                                    <label className="text-xs text-text-secondary font-medium mb-1.5 block">Hora *</label>
                                    <input type="time" value={visitaHora} onChange={e => setVisitaHora(e.target.value)} className="w-full min-h-[38px] px-3 bg-bg-secondary border border-border rounded-lg text-sm" />
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="text-xs text-text-secondary font-medium mb-1.5 block">Descripción detallada</label>
                            ...
                            <div className="flex gap-3 mt-1">
                                ...
                                <button type="button" onClick={handleSubmit} disabled={!visitaTecnico || !visitaFecha || !visitaHora} className="flex-1 py-2.5 rounded-lg bg-accent-purple border-none text-white cursor-pointer text-sm font-bold hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed">
                                    Agendar Visita
                                </button>
                            </div>
                        </div>
